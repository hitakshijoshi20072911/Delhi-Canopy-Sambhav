...
import { useClimateTrends } from "@/hooks/useDashboardData";
...
export function ClimateTrendChartLive() {
  const { trends, isLoading, error, refresh } = useClimateTrends(12);
  ...
  const chartData = useMemo(() => {
    if (trends.length > 0) {
      return trends.map(t => ({
        ...
        greenCover: t.greenCover,
      }));
    }
    return monthlyHeatData;
  }, [trends]);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground font-tech">
            {trends.length > 0 ? 'LIVE DATA • 12 MONTHS' : 'TEMPERATURE VS GREEN COVER • 12 MONTHS'}
          </p>
        </div>
      ...
      <div className="h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <defs>
              <linearGradient id="heatGradientLive" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="hsl(0, 84%, 60%)" stopOpacity={0.4} />
                <stop offset="95%" stopColor="hsl(0, 84%, 60%)" stopOpacity={0} />
              </linearGradient>
              <linearGradient id="greenGradientLive" x1="0" y1="0" x2="0" y2="1">
